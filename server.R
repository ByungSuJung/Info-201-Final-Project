...
correlation <- function(corr) {
  ...
  if(-0.7 > corr | corr > 0.7) {
    return(paste0(" The correlation with ", corr, " have very <i>strong correlation.</i>"))
  } else if (-0.3 > corr | corr > 0.3){
    return(paste0(" The correlation with ", corr, " is <i>moderate correlation.</i>"))
  } else {
    return(paste0(" The correlation with ", corr, " is <i>weak correlation.</i>"))
  }
}
...
shinyServer(function(input, output) {
  ...
  # Render relation between xasix and yaxis.
  output$correlation <- renderText({
    
    filtered.data <-  filter(soccer.data, Overall >= as.numeric(input$Overall)) %>% 
      select(input$xaxis, input$yaxis)
    
    corr <- cor(filtered.data[sapply(filtered.data,is.numeric)])[1,2]

    return(paste0("<h2>The Relation between ", input$xaxis, " and ", input$yaxis, " is ", round(corr,3),". </h2> <br/>",
                  "<h3>", correlation(round(corr,3)), "<h3>"))
  })
